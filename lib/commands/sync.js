...
module.exports = function *(argv) {
  ...
  for (let table of tablesToSync) {
    ...
    let sourceCursor = yield sr.db(sourceDB).table(table).orderBy({index: sr.asc('id')})
      .map(function (row) { return {id: row('id'), type: sr.typeOf(row('id')), hash: sr.uuid(row.toJSON())} })
      .run({cursor: true})
    let targetCursor = yield tr.db(targetDB).table(table).orderBy({index: tr.asc('id')})
      .map(function (row) { return {id: row('id'), type: tr.typeOf(row('id')), hash: tr.uuid(row.toJSON())} })
      .run({cursor: true})
    ...
    while (si.id !== maxId || ti.id !== maxId) {
      let cmp = si.type === ti.type ? compareValues(si.id, ti.id) : compareValues(si.type, ti.type)
      if (cmp == 0) {
        let sid = si.id
        let tid = ti.id
        if (si.hash !== ti.hash) {
...
const compareValues = (a, b) => {
  if (_.isArray(a) && _.isArray(b)) {
    let diff = _.head(_.dropWhile(_.zip(a, b), (pair) => _.isEqual(pair[0], pair[1])))
    return _.isUndefined(diff) ? 0 : compareValues(diff[0], diff[1])
  } else if (_.isObject(a) && _.isObject(b)) {
    return compareValues(_.toPairs(a).sort(), _.toPairs(b).sort())
  } else {
    return _.isEqual(a, b) ? 0 : (_.lt(a, b) ? -1 : 1)
  }
}
